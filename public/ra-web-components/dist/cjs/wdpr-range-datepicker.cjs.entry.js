...
var wdprDatepicker_model = require('./wdpr-datepicker.model-BC4Hiu8a.js');
var bundleCjs = require('./bundle-cjs-Cajw0YnV.js');
require('./index-oP7sywWN.js');
...
const WdprRangeDatePicker = class {
     ...
     * @type {string}
     */
    min = date_utils.formatDateToString(new Date(0, 0, 1), this.format);
    /**
    ...
    componentWillLoad() {
        this._internalDialogId = this.dialogId || `wdpr-range-datepicker-calendar-${utils.generateRandId()}`;
    ...
    _handleRangeDatesChanged = (event) => {
        ...
        if (startDate && !date_utils.isValidDateString(startDate, this.format)) {
            this.dateInvalid.emit({ type: 'invalid-format', start: startDate });
            return;
        }
        if (endDate && !date_utils.isValidDateString(endDate, this.format)) {
            this.dateInvalid.emit({ type: 'invalid-format', end: endDate });
            return;
        }
        const parsedStart = startDate ? date_utils.parseDateFromString(startDate, this.format) : undefined;
        const parsedEnd = endDate ? date_utils.parseDateFromString(endDate, this.format) : undefined;
        const parsedMin = this.min ? date_utils.parseDateFromString(this.min, this.format) : undefined;
        const parsedMax = this.max ? date_utils.parseDateFromString(this.max, this.format) : undefined;
        if (parsedStart && ((parsedMin && date_utils.isBeforeDay(parsedStart, parsedMin)) || (parsedMax && date_utils.isAfterDay(parsedStart, parsedMax)))) {
            this.dateInvalid.emit({ type: 'out-of-range', start: startDate });
            return;
        }
        if (parsedEnd && ((parsedMin && date_utils.isBeforeDay(parsedEnd, parsedMin)) || (parsedMax && date_utils.isAfterDay(parsedEnd, parsedMax)))) {
            this.dateInvalid.emit({ type: 'out-of-range', end: endDate });
            return;
        }
        if (parsedStart && parsedEnd && date_utils.isAfterDay(parsedStart, parsedEnd)) {
    ...
    _handleInputKeyDown = async (ev) => {
        ...
        if (ev.key === keycodes_model.KEYBOARD_KEYS.ARROW_DOWN) {
            ...
            if (this._calendarElement) {
                ...
                this._calendarElement.setFocusDate(date_utils.parseDateFromString(focusDate, this.format));
            }
        }
    ...
    _getTodayString() {
        ...
        return date_utils.formatDateToString(today, this.format);
    }
    _getDateExtras() {
    ...
    get _calendarWrapperClasses() {
        ...
        return bundleCjs.bundleCjsExports.twMerge(baseClasses, shadowClass, _expandedClasses);
    }
    get _textFieldWrapperClasses() {
        if (this.inputMaxWidth)
            return 'w-full';
        return bundleCjs.bundleCjsExports.twMerge('m-auto md:w-[358px]', wdprDatepicker_model.wrapperTextFieldClasses[this.calendarSize]);
    }
    render() {
        return (index.h("div", { key: '74eaf8b7202713007feaa38964d2891b7c26dd54', class: "relative" }, index.h("div", { key: 'b11fcde63aa1226f338628887cf31e6ac4d61bf7', class: this._textFieldWrapperClasses }, index.h("wdpr-dual-text-field", { key: '372fa1c445bc71b550d23cb560bb606b36d740e3', ref: el => (this._dualTextFieldElement = el), leadingLabel: this.startLabel, trailingLabel: this.endLabel, leadingValue: this.start, trailingValue: this.end, error: this.error, disabled: this.disabled, helperText: this.helperText, readonly: this.readonly, a11yRole: "combobox", a11yHasPopup: "dialog", a11yControls: this._internalDialogId, a11yExpanded: this._expanded ? 'true' : 'false', onWdprInputClick: this._handleInputClick, onWdprValueChanged: this._handleRangeDatesChanged, onKeyDown: this._handleInputKeyDown }, !this.readonly && (index.h("wdpr-icon-button", { key: '5abc28cb52075b6c18be0aea9386fbdacf532d7e', size: "medium", iconName: "calendar-month", slot: "trailing-icon-button", a11yLabel: "Choose Dates", disabled: this.disabled, onClicked: this._toggleExpandCalendar, customTabIndex: -1 })))), index.h("div", { key: '23e251e0fe57038815f350da582296542bb5506a', id: this._internalDialogId, class: this._calendarWrapperClasses, part: "calendar-wrapper", role: "dialog", "aria-label": "Choose Date" }, index.h("wdpr-calendar", { key: 'cc3854e9c3c5576afb8bfd4bc5e53926ae4e59e7', ref: el => (this._calendarElement = el), mode: "range", enableSixWeeks: this.enableSixWeeks, start: this.start, end: this.end, format: this.format, min: this.min, max: this.max, variant: this.variant, displayBorder: this.displayCalendarBorder, rangeActiveField: this._activeField, onRangeDatesSelected: this.handleRangeDatesSelected, onKeyDown: this._handleCalendarKeyDown, exportparts: "calendar, calendar-surface", size: this.calendarSize }))));
    }
...
exports.wdpr_range_datepicker = WdprRangeDatePicker;
//# sourceMappingURL=wdpr-range-datepicker.entry.cjs.js.map

//# sourceMappingURL=wdpr-range-datepicker.cjs.entry.js.map