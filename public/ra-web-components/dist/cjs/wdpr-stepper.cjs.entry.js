...
const WdprStepper = class {
    get el() { return index.getElement(this); }
    _internals;
    _inputId = `stepper-${utils.generateRandId()}`;
    _alertId = `alert-${utils.generateRandId()}`;
    _errorId = `error-${utils.generateRandId()}`;
    _defaultValue = 0;
    _inputRef;
    constructor(hostRef) {
        index.registerInstance(this, hostRef);
        this.updateDispatcher = index.createEvent(this, "updated", 3);
        this.invalidValueDispatcher = index.createEvent(this, "invalidValue", 7);
    ...
    _handleKeyDown = (event) => {
        const key = event.key;
        // Allow Tab
        if (key === keycodes_model.KEYBOARD_KEYS.TAB)
            return;
        // Check if it's a number key (0-9)
        const isNumber = /^[0-9]$/.test(key);
        if (!this.nonTextEditable) {
            switch (key) {
                case keycodes_model.KEYBOARD_KEYS.PAGE_UP:
                    event.preventDefault();
                    this.value = Math.min(this.value + 5, this.upperLimit);
                    break;
                case keycodes_model.KEYBOARD_KEYS.PAGE_DOWN:
                    event.preventDefault();
                ...
                default:
                    ...
                    const allowedKeys = [
                        keycodes_model.KEYBOARD_KEYS.BACKSPACE,
                        keycodes_model.KEYBOARD_KEYS.DELETE,
                        keycodes_model.KEYBOARD_KEYS.ARROW_LEFT,
                        keycodes_model.KEYBOARD_KEYS.ARROW_RIGHT,
                        keycodes_model.KEYBOARD_KEYS.ARROW_UP,
                        keycodes_model.KEYBOARD_KEYS.ARROW_DOWN,
                    ];
            ...
            if (key !== keycodes_model.KEYBOARD_KEYS.ARROW_UP && key !== keycodes_model.KEYBOARD_KEYS.ARROW_DOWN && key !== keycodes_model.KEYBOARD_KEYS.TAB) {
                event.preventDefault();
            }
        }
        // Arrow keys for increment/decrement
        if (key === keycodes_model.KEYBOARD_KEYS.ARROW_UP && !this.plusDisabled && this.value < this.upperLimit) {
            event.preventDefault();
            this.value += 1;
        }
        else if (key === keycodes_model.KEYBOARD_KEYS.ARROW_DOWN && !this.minusDisabled && this.value > this.lowerLimit) {
            event.preventDefault();
            this.value -= 1;
        }
    };
    ...
    _emitNativeEvents() {
        const changeEvent = utils.createEvent('change', {
            bubbles: true,
            composed: true,
        });
    ...
    _getInputClasses() {
        ...
        return utils.customTwMerge(baseClasses, focusClasses, disabledClasses);
    }
    _getCounterClasses() {
    ...
    _getContainerClasses() {
        ...
        return utils.customTwMerge(baseClasses, hiddenButtons, sizeClass);
    }
    ...
    _getHeaderClasses() {
        ...
        return utils.customTwMerge(sizeClass, baseClasses);
    }
    ...
    render() {
        ...
        const stepperControls = (index.h("div", { key: '9c790a48f695825b569741f09a735e7623baf323', class: this._getStepperControlsClasses() }, index.h("wdpr-icon-button", { key: 'e885034055a2f0a96146d0b93a93cf3915bffbab', type: "button", variant: "primary", size: buttonSize, iconName: "decrease", disabled: isDecrementDisabled, onClicked: this._decrement, a11yLabel: this.decrementLabel, tabindex: -1 }), this.nonTextEditable ? (index.h("div", { class: this._getCounterClasses(), role: "spinbutton", tabindex: 0, onKeyDown: this._handleKeyDown, "aria-label": this.inputAriaLabel, "aria-valuenow": this.value, "aria-valuemin": this.lowerLimit, "aria-valuemax": this.upperLimit, "aria-valuetext": this.value.toString() }, this.value)) : (index.h("input", { ref: this._handleInputRef, type: "number", id: this._inputId, min: this.lowerLimit, max: this.upperLimit, step: "1", class: this._getInputClasses(), value: this.value, disabled: this.disabled, "aria-label": this.inputAriaLabel, "aria-describedby": ariaDescribedBy ? 'area-description' : undefined, "aria-invalid": !this.valid, onKeyDown: this._handleKeyDown, onChange: this._handleChange })), index.h("wdpr-icon-button", { key: '804b1bdfe2ffc6b04c926998b13efb0de90604e4', type: "button", variant: "primary", size: buttonSize, iconName: "increase", disabled: isIncrementDisabled, onClicked: this._increment, a11yLabel: this.incrementLabel, tabindex: -1 }), this.fieldName && index.h("input", { key: '67df38e896290644a36e823ce39e14b829192d69', type: "hidden", name: this.fieldName, value: this.value })));
        return (index.h("div", { key: '2267118e28f7a3293319bb637c19439730880fef' }, index.h("div", { key: '92b495e5e71c7fad002e65734c2dc1c124a016ea', class: this._getContainerClasses() }, (this.header || this.subHeader) && (index.h("div", { key: 'ba4139c348c9dc6dfc8444970ef292ce1c80a888', class: this._getLabelContainerClasses() }, this.header && index.h("h3", { key: '985f86b9d1f1832ad82a44c9c5f24260dcebde88', class: this._getHeaderClasses() }, this.header), this.subHeader && index.h("p", { key: '9d77b4dfb6079ba243ba74e91f9109b9d3d67699', class: this._getSubHeaderClasses() }, this.subHeader))), stepperControls), showError && (index.h("div", { key: '92193222edd0ab3e1cdf4d06e1fea2060ee6b78f', class: "block p-150 w-full" }, index.h("wdpr-inline-message", { key: 'e7e7203243406d64fb06c998974d6afb37083bbf', variant: "error", size: "small" }, this.errorMessage))), index.h("span", { key: '76caf8e15f02ea06276a379499a84b71e594af76', id: this._alertId, class: "sr-only", "aria-live": "assertive", "aria-atomic": "true" }), ariaDescribedBy && (index.h("span", { key: 'abf1cd531689d401bbc2794bbf09cc2a8951f2e7', id: "area-description", class: "sr-only" }, ariaDescribedBy))));
    }
...
exports.wdpr_stepper = WdprStepper;
//# sourceMappingURL=wdpr-stepper.entry.cjs.js.map

//# sourceMappingURL=wdpr-stepper.cjs.entry.js.map