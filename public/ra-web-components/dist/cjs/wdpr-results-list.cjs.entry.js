...
const WdprResultsList = class {
    ...
    _trackShiftKey = (ev) => {
        if (ev.key === keycodes_model.KEYBOARD_KEYS.SHIFT) {
    ...
    handleKeyDown(ev) {
        ...
        if (allItems.length === 0)
            return;
        switch (ev.key) {
            case keycodes_model.KEYBOARD_KEYS.TAB:
    ...
    _renderListItem(item, index$1, totalItems) {
        ...
        const isFocused = this._focusedIndex === index$1 && this._showFocusRing;
        ...
        return (index.h("div", { key: item.id }, index.h("wdpr-results-list-item", { variant: variant, label: item.label, description: item.description, icon: item.icon, selected: isSelected, disabled: item.disabled, checked: isChecked, value: item.value, itemId: item.id, isFocused: isFocused, onWdprSelect: e => this._handleItemSelect(e, item, index$1), onWdprChange: e => this._handleItemSelect(e, item, index$1) }), shouldShowDivider && (index.h("div", { class: "mx-100" }, index.h("wdpr-divider", null)))));
    }
    _renderGroupHeader(label) {
        return (index.h("div", { class: "px-050 py-100" }, index.h("span", { class: "body-medium-alt text-text-heading" }, label)));
    }
    _renderGroups() {
        let globalIndex = 0;
        return (index.h("div", { class: utils.customTwMerge('flex flex-col', this._getSpacingClass()) }, this.groups.map(group => (index.h("div", { key: group.id }, this._renderGroupHeader(group.label), index.h("div", { class: utils.customTwMerge('flex flex-col', this._getSpacingClass()) }, group.items.map((item, itemIndex) => {
    ...
    _renderGroupListItem(item, globalIndex, itemIndex, groupItemCount) {
        ...
        // For checkbox variant, use _selectedItems state for checked; otherwise use item.checked
        const isChecked = this.selectionMode === 'multiple' ? isSelected : item.checked;
        return (index.h("div", { key: item.id }, index.h("wdpr-results-list-item", { variant: variant, label: item.label, description: item.description, icon: item.icon, selected: isSelected, disabled: item.disabled, checked: isChecked, value: item.value, itemId: item.id, isFocused: isFocused, onWdprSelect: e => this._handleItemSelect(e, item, globalIndex), onWdprChange: e => this._handleItemSelect(e, item, globalIndex) }), shouldShowDivider && (index.h("div", { class: "mx-100" }, index.h("wdpr-divider", null)))));
    }
    _renderItems() {
        return index.h("div", { class: utils.customTwMerge('flex flex-col', this._getSpacingClass()) }, this.items.map((item, index) => this._renderListItem(item, index, this.items.length)));
    }
    _renderContent() {
        if (this.groups && this.groups.length > 0) {
    ...
    render() {
        ...
        return (index.h(index.Host, { key: '5e3db383c8b8522d09eda2f4ce2022c50e64226d', role: "listbox", tabindex: 0, "aria-labelledby": this.labelledBy, onFocus: this._handleListFocus, onBlur: this._handleListBlur, onPointerDown: this._handlePointerDown, onClick: this._handleClick, part: "results-list" }, !this.isLoading && !this.errorOnLoad && !this.isEmpty && (index.h("wdpr-surface-style", { key: '6f51693b5e1e8a00b7169e78ea8d330a373bfa62', variant: "basic", customClass: "elevation-xsmall-soft p-200" }, needsScrollbar ? (index.h("div", { style: { height: this._measuredMaxHeight } }, index.h("wdpr-scrollbar", { orientation: "vertical", size: "small" }, this._renderContent()))) : (this._renderContent()), index.h("slot", { key: '77411825a1c421347f325be670a30c34c8afbb59' }))), this.isLoading && this.loadingText && (index.h("wdpr-surface-style", { key: '74ed9f5e0e9ab762a274211550fc5c6a9da021a4', variant: "basic", customClass: "elevation-xsmall-soft py-300 px-200 flex flex-col items-center justify-center" }, index.h("wdpr-radial-loader", { key: '559c5090a5f7ff80f04fd923cd644216f49d5ce1', label: this.loadingText, size: this.loaderSize, labelPlacement: this.loaderLabelPlacement, showLabel: this.showLoaderLabel }))), !this.isLoading && this.isEmpty && this.emptyText && (index.h("wdpr-surface-style", { key: '92750c77cb0ba37201d52cb0f7e68e966b4a7bd5', variant: "basic", customClass: "elevation-xsmall-soft py-600 px-200" }, index.h("div", { key: '2ada71bf5e6e41945eec6b73a0bf58f6784167dd', class: "flex flex-col items-center justify-center gap-100 text-text-disabled" }, index.h("wdpr-icon-library", { key: '0afcab0e0b333c6df36b7f78a67ca5434658da4c', icon: this.emptyIcon, size: this.emptyIconSize, decorative: true }), index.h("p", { key: 'aa84c7e46c5b8602bad24e066412479729197961', class: "text-body-large text-text-disclaimer" }, this.emptyText)))), !this.isLoading && this.errorOnLoad && this.errorText && (index.h("wdpr-surface-style", { key: '5b16e58a969ac153bcdf1bdfaa5491f9b35e9ab3', variant: "basic", customClass: "elevation-xsmall-soft py-600 px-200 flex flex-col items-center justify-center" }, index.h("wdpr-inline-message", { key: '5dd594828995da9925a786bd287bb72ebd320a08', variant: "error", size: this.errorTextSize }, this.errorText)))));
    }
...
WdprResultsList.style = ":host {\n      display: block;\n      outline: none;\n    }";

exports.wdpr_results_list = WdprResultsList;
//# sourceMappingURL=wdpr-results-list.entry.cjs.js.map

//# sourceMappingURL=wdpr-results-list.cjs.entry.js.map