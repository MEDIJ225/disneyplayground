...
const WdprDate = class {
    ...
    componentDidRender() {
        ...
        const key = this.cellData && date_utils.formatDateToString(this.cellData.date, this.format);
    ...
    _onDateHover = () => {
        if (!this.cellData || this.cellData.disabled || this.isReadOnly)
            return;
        this.dateHover.emit({ date: date_utils.formatDateToString(this.cellData.date, this.format) });
    };
    _onDateClick = () => {
        if (!this.cellData || this.cellData.disabled || this.isReadOnly)
            return;
        this.dateSelected.emit({ date: date_utils.formatDateToString(this.cellData.date, this.format) });
    };
    _onKeyDown = (ev) => {
        if (!this.cellData || this.cellData.disabled || this.isReadOnly)
            return;
    ...
    get _ariaLabel() {
        if (!this.cellData)
            return;
        const formattedDate = date_utils.formatDateToString(this.cellData.date, this.format);
    ...
    get _wrapperClasses() {
        ...
        return utils.customTwMerge(baseClasses, range, firstDate, endDate);
    }
    get _containerClasses() {
        if (!this.cellData)
            return utils.customTwMerge(containerBaseClasses$1);
        const isEnabled = !this.cellData.disabled;
        return utils.customTwMerge(containerBaseClasses$1, containerActiveDateClasses, this.cellData && this.cellData.inRange && isEnabled ? containerInRangeDateClasses : null, this.cellData && this.cellData.isStartDate && isEnabled ? containerStartDateClasses : null, this.cellData && this.cellData.isEndDate && isEnabled ? containerEndDateClasses : null, this.isFocusable ? 'z-10' : null);
    }
    get _dateClasses() {
        if (!this.cellData)
            return utils.customTwMerge(dateBaseClasses);
        if (this.isReadOnly)
            return utils.customTwMerge(dateBaseClasses, readOnlyDateClasses);
        const isEnabled = !this.cellData.disabled;
        return utils.customTwMerge(dateBaseClasses, this.cellData && this.cellData.disabled ? disabledDateClasses : activeDateClasses, this.cellData && isEnabled && (this.cellData.selected || this.cellData.isStartDate || this.cellData.isEndDate) ? selectedDateClasses : null, this.cellData && isEnabled && this.cellData.inRange ? inRangeDateClasses : null);
    }
    get _dotClasses() {
        return bundleCjs.bundleCjsExports.twMerge(this.cellData.selected || this.cellData.isStartDate || this.cellData.isEndDate ? 'bg-surface-default' : '');
    }
    render() {
        const key = this.cellData && date_utils.formatDateToString(this.cellData.date, this.format);
        ...
        return (index.h("div", { key: 'd486390bf6ae89f02a811129d2b18645a87c2ccb', class: this._wrapperClasses, part: "date-cell-wrapper" }, index.h("div", { key: '1f37dceff1611b7982014be9783c71b1ec206971', role: "gridcell", class: this._containerClasses, onClick: this._onDateClick, onKeyDown: this._onKeyDown, onMouseEnter: this._onDateHover, tabIndex: this.isFocusable ? 0 : -1, "aria-label": this.cellData && this._ariaLabel, "aria-describedby": [priceId, this.cellData && this.cellData.showDot ? `dot-desc-${safeKey}` : undefined].filter(Boolean).join(' ') || undefined, "aria-disabled": !this.cellData || this.cellData.disabled ? 'true' : null, "aria-selected": this.cellData && (this.cellData.selected || this.cellData.isStartDate || this.cellData.isEndDate) ? 'true' : null, part: "date-cell" }, this.cellData && (index.h("span", { key: 'f11d227cefbbf19bdac3af338e2e3bb6fd4f5c72', part: "date-label", class: this._dateClasses }, this.cellData.disabled && this.showDisabledSlash && !this.cellData.isOutsideMonth && (index.h("span", { key: '817f7435eea96e3e454ec3a79903e3625a4a1159', class: "absolute left-[10px] top-[10px]" }, index.h("wdpr-icon-library", { key: '184d4997bf1137912ba0564b326ea6c99d973bb3', size: "medium", icon: "calendar-icon" }))), this.cellData.date.getDate(), this.cellData.showDot && (index.h(index.Fragment, { key: '388c64090733ff0dfb4a41ac305081eff81b0575' }, index.h("div", { key: 'd513877612d7c66443e097742e83e4c0310114df', class: "absolute bottom-075", part: "date-cell-dot" }, index.h("wdpr-calendar-dot", { key: '3066750a388da7ecbe0ddede39a8d9556ed5a206', dotStyle: this._dotClasses })), index.h("span", { key: '27214cf2781a0b4bb4d9ca7969fe3fc55b4f0d40', id: `dot-desc-${safeKey}`, class: "sr-only" }, "This day is marked with an indicator dot.")))))), this.cellData && this.cellData.price && (index.h("span", { key: 'ebda06f2fd544c76bd2e0db811800f9bff6366ae', id: priceId, class: "label-xsmall text-text-label py-050", part: "date-cell-price" }, index.h("span", { key: '199ee42832e0f27273b5a3737df0b8eeda56b927', class: "sr-only" }, "Price: "), parsedPrice)), index.h("div", { key: 'fd8f7864d207ad7c562b02447203867c10c7988e', class: utils.customTwMerge(hasExtra ? 'my-050' : ''), ref: el => (this._extraContainer = el), part: "date-cell-extra-info" })));
    }
};
...
const WdprMonth = class {
    ...
    _getCellDates() {
        ...
        for (let i = 1; i <= daysInMonth; i++) {
            ...
            if (this.mode === 'single') {
                cellDates.push({
                    date: currentDate,
                    selected: index$1.dateFnsExports.isSameDay(currentDate, this.selectedDate),
                    isStartDate: false,
                    isEndDate: false,
                    inRange: false,
                    disabled: date_utils.isBeforeDay(currentDate, this.minDate) || date_utils.isAfterDay(currentDate, this.maxDate) || isBlocked,
                    showDot: isDot,
                    price: price,
                });
            }
            if (this.mode === 'range') {
                let isInRange = false;
                let isEndDate = false;
                const isDisabled = date_utils.isBeforeDay(currentDate, this.minDate) || date_utils.isAfterDay(currentDate, this.maxDate) || date_utils.isAfterDay(currentDate, this.maxRangeDate) || isBlocked;
                if (this.startDate && this.endDate) {
                    isInRange = date_utils.isAfterDay(currentDate, this.startDate) && date_utils.isBeforeDay(currentDate, this.endDate);
                    isEndDate = index$1.dateFnsExports.isSameDay(currentDate, this.endDate);
                }
                if (this.startDate && !this.endDate && this.hoverDate) {
                    const min = date_utils.isAfterDay(this.hoverDate, this.startDate) ? this.startDate : this.hoverDate;
                    const max = date_utils.isBeforeDay(this.hoverDate, this.startDate) ? this.startDate : this.hoverDate;
                    isInRange = (date_utils.isAfterDay(currentDate, min) || index$1.dateFnsExports.isSameDay(currentDate, min)) && (date_utils.isBeforeDay(currentDate, max) || index$1.dateFnsExports.isSameDay(currentDate, min));
                    isEndDate = index$1.dateFnsExports.isSameDay(currentDate, this.hoverDate);
                }
                cellDates.push({
                    date: currentDate,
                    selected: false,
                    isStartDate: index$1.dateFnsExports.isSameDay(currentDate, this.startDate),
    ...
    _handleKeyDown = (ev) => {
        ...
        switch (ev.key) {
            case keycodes_model.KEYBOARD_KEYS.ARROW_RIGHT:
                newFocusDate = index$1.dateFnsExports.addDays(this.focusDate, 1);
                break;
            case keycodes_model.KEYBOARD_KEYS.ARROW_LEFT:
                newFocusDate = index$1.dateFnsExports.subDays(this.focusDate, 1);
                break;
            case keycodes_model.KEYBOARD_KEYS.ARROW_DOWN:
                newFocusDate = index$1.dateFnsExports.addWeeks(this.focusDate, 1);
                break;
            case keycodes_model.KEYBOARD_KEYS.ARROW_UP:
                newFocusDate = index$1.dateFnsExports.subWeeks(this.focusDate, 1);
                break;
            default:
                return;
        }
        ev.preventDefault();
        if ((this.minDate && date_utils.isBeforeDay(newFocusDate, this.minDate)) || (this.maxDate && date_utils.isAfterDay(newFocusDate, this.maxDate))) {
            return;
        }
        this.focusMoveOut.emit({ date: date_utils.formatDateToString(newFocusDate, this.format) });
    };
    _focusCellDate() {
        if (!this.focusDate)
            return;
        const index = this._cellDates.findIndex(cell => cell && index$1.dateFnsExports.isSameDay(cell.date, this.focusDate));
        if (index === -1)
            return;
    ...
    _isFocusableCell(cellData) {
        ...
        return !!this.focusDate && index$1.dateFnsExports.isSameDay(cellData.date, this.focusDate) && !(date_utils.isBeforeDay(cellData.date, this.minDate) || date_utils.isAfterDay(cellData.date, this.maxDate) || blocked);
    }
    _getWeeks(cellDates) {
        const weeks = [];
...
const WdprMonthHeader = class {
    constructor(hostRef) {
        index.registerInstance(this, hostRef);
        this.previousMonthClick = index.createEvent(this, "previousMonth", 7);
        this.nextMonthClick = index.createEvent(this, "nextMonth", 7);
    }
    get el() { return index.getElement(this); }
    ...
    validateButtons() {
        ...
        const nextMonth = date_utils.getNextMonth(this.displayDate);
        ...
        this._isNextMonthDisabled = this.maxDate && date_utils.isAfterDay(firstDayOfNextMonth, this.maxDate);
    }
    setTitle() {
    ...
    render() {
        ...
        return (index.h("section", { key: 'b6a3a805fab81c8c808f75da1ecc53ad7a221ebd', part: "month-header", class: "flex items-center py-100 justify-between" }, withArrows && (index.h("wdpr-icon-button", { key: '3566798148067d842435fd5d2e9995947fc42424', variant: "primary", iconName: "previous", a11yLabel: "Previous Month", disabled: this._isPrevMonthDisabled, onClicked: this.onPreviousMonth })), index.h("h2", { key: '27c572cad1843e0f7a169b47ebb7eac69f6e22d7', class: "m-0 flex items-center gap-100", "aria-live": "polite", "aria-atomic": "true", part: "month-header-title" }, index.h("span", { key: '86c2e9895e91567e98d849593e9f400c234d3742', class: "heading-small !text-text-body" }, this._month), index.h("span", { key: 'd9a02a7d1f8146c3271e61df3c86fb99419374fb', class: "heading-small-alt !text-text-body" }, this._year)), withArrows && index.h("wdpr-icon-button", { key: '7df2f363c102fdf99dd6a3ef6062dd6cd3d16035', variant: "primary", iconName: "next", a11yLabel: "Next Month", disabled: this._isNextMonthDisabled, onClicked: this.onNextMonth })));
    }
    static get watchers() { return {
...
const WdprSingleCalendar = class {
    ...
    get _wrapperClasses() {
        ...
        return utils.customTwMerge(wrapperBaseClasses, widthClass);
    }
    get _containerClasses() {
        return utils.customTwMerge(containerBaseClasses, this.displayBorder ? containerBorderClasses : null);
    }
    render() {
        return (index.h(index.Host, { key: '2c39bf06e22bf01f77999a95a8e71f52eea79f50', class: { 'full-width': this.fullWidth } }, index.h("div", { key: 'd8fda6c0a19d646f338f31675052a7be50f44c16', part: "calendar", class: this._wrapperClasses }, index.h("wdpr-month-header", { key: '2e2582a4006dd4517286bd9e23f3ba3f50688ce0', id: "month-title", displayDate: this.displayDate, variant: this.monthHeaderVariant, minDate: this.minDate, maxDate: this.maxDate, onNextMonth: this._onNextMonth, onPreviousMonth: this._onPreviousMonth }), index.h("div", { key: '90c802cab46a75693103538244f2911356409974', part: "calendar-surface", class: this._containerClasses, role: "grid", "aria-rowcount": this.enableSixWeeks ? '7' : '6', "aria-colcount": "7", "aria-label": `${this.displayDate.toLocaleString('default', { month: 'long' })} ${this.displayDate.getFullYear()}` }, index.h("wdpr-week-header", { key: '4ee16426460025c870af0b762ebb3cea281a6676', fullWidth: this.fullWidth }), index.h("div", { key: '437b646e71b824f2cbfef90dcd0cbff0656960bd', role: "row", "aria-hidden": "true" }, index.h("wdpr-divider", { key: 'aadf12736dbfa181d1f19110353359d2653036bd' })), index.h("wdpr-month", { key: 'e96f5ec445077eacbf58723f5ce386a53f1e6691', mode: this.mode, enableSixWeeks: this.enableSixWeeks, startDate: this.startDate, endDate: this.endDate, hoverDate: this.hoverDate, focusDate: this.focusDate, selectedDate: this.selectedDate, displayDate: this.displayDate, format: this.format, minDate: this.minDate, maxDate: this.maxDate, extras: this.extras, showDisabledSlash: this.showDisabledSlash, maxRangeDate: this.maxRangeDate, fullWidth: this.fullWidth, isReadOnly: this.isReadOnly, isDateBlocked: this.isDateBlocked, isDateDot: this.isDateDot, getDatePrice: this.getDatePrice, onFocusMoveOut: this._onFocusMoveOut, onDateSelected: this._onDateSelected, onDateHover: this._onDateHover })))));
    }
};
...
const WdprWeekHeader = class {
    ...
    get _wrapperClasses() {
        ...
        return bundleCjs.bundleCjsExports.twMerge('flex flex-row label-medium !text-text-disclaimer', width);
    }
    render() {
        const localizedWeekdays = this.getLocalizedWeekdays();
        return (index.h("div", { key: 'e284d4dd2df1b63b54e98498e4f14bdadc7c2b3d', class: this._wrapperClasses, part: "week-header-wrapper", role: "row" }, localizedWeekdays.map(day => (index.h("span", { class: "flex-1 py-050 text-center", part: "week-header-label", role: "columnheader" }, index.h("span", { class: "sr-only" }, day), index.h("span", { "aria-hidden": "true" }, day[0]))))));
    }
};

exports.wdpr_date = WdprDate;
exports.wdpr_month = WdprMonth;
exports.wdpr_month_header = WdprMonthHeader;
exports.wdpr_single_calendar = WdprSingleCalendar;
exports.wdpr_week_header = WdprWeekHeader;
//# sourceMappingURL=wdpr-date.wdpr-month.wdpr-month-header.wdpr-single-calendar.wdpr-week-header.entry.cjs.js.map

//# sourceMappingURL=wdpr-date_5.cjs.entry.js.map