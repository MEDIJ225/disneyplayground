...
function renderTags(config) {
    ...
    return (index.h(index.Fragment, null,
        visibleTags.map((item) => (index.h("wdpr-combobox-tag", { key: item.id, label: item.label }))),
        overflowCount > 0 && (index.h("span", { class: "inline-flex items-center justify-center h-250 px-100 body-medium-alt text-text-status-neutral bg-surface-status-neutral rounded-050 shrink-0" },
            "+",
            overflowCount))));
}
/**
...
const WdprCombobox = class {
    ...
    componentWillLoad() {
        this._listId = `wdpr-combobox-list-${utils.generateRandId()}`;
        this._textFieldId = `wdpr-text-field-${utils.generateRandId()}`;
    }
    componentDidLoad() {
        requestAnimationFrame(() => {
            this._initAllItems();
    ...
    _handleTagNavigation = async (ev) => {
        ...
        if (ev.key === keycodes_model.KEYBOARD_KEYS.ARROW_LEFT) {
        ...
        if (ev.key === keycodes_model.KEYBOARD_KEYS.ARROW_RIGHT && this._highlightedChipIndex >= 0) {
            ev.preventDefault();
        ...
        if (ev.key === keycodes_model.KEYBOARD_KEYS.BACKSPACE || ev.key === keycodes_model.KEYBOARD_KEYS.ENTER || ev.key === keycodes_model.KEYBOARD_KEYS.SPACE) {
            if (this._highlightedChipIndex >= 0) {
                ev.preventDefault();
            ...
            else if (this._inputValue === '' && tagCount > 0 && ev.key === keycodes_model.KEYBOARD_KEYS.BACKSPACE) {
                ev.preventDefault();
                this._highlightedChipIndex = tagCount - 1;
                await this._focusTag(this._highlightedChipIndex);
                return true;
        ...
        if (this._highlightedChipIndex >= 0 && ev.key !== keycodes_model.KEYBOARD_KEYS.ESCAPE && ev.key !== keycodes_model.KEYBOARD_KEYS.TAB) {
            this._highlightedChipIndex = -1;
            this._focusInput();
        }
        return false;
    ...
    _renderMultiSelectInput() {
        ...
        return (index.h("div", { class: "multi-selection-wrapper" }, index.h("div", { ref: el => (this._tagsContainerRef = el), class: `multi-selection-container ${hasContent ? 'has-tags' : ''}` }, index.h("div", { class: `multi-selection-tags-input-wrapper ${hasContent ? 'tags-input-floated' : ''}` }, this._renderTags(), index.h("input", { type: "text", value: this._inputValue, placeholder: " ", id: this._textFieldId, class: "multi-selection-input", disabled: this.disabled, name: this.name, tabindex: this.disabled ? -1 : 0, readonly: this.readonly, "aria-invalid": this.error.toString(), role: "combobox", "aria-haspopup": "listbox", "aria-autocomplete": "list", "aria-expanded": this._expanded.toString(), "aria-controls": this._listId, "aria-disabled": this.disabled.toString(), "aria-activedescendant": this._activeDescendantId, "aria-required": this.required.toString(), "aria-describedby": this.helperText ? `${this._textFieldId}-helper-text` : undefined, onClick: this._handleInputClick, onInput: this._handleInput }), index.h("label", { class: `multi-selection-input-label ${hasContent ? 'label-floated' : 'label-not-floated'}`, htmlFor: this._textFieldId, innerHTML: this.label })), !this.hideIcon && !this.readonly && (index.h("span", { class: "icon-wrapper" }, index.h("wdpr-icon-button", { buttonId: this._listId, iconName: this._getIconName(), a11yLabel: this._getIconButtonAriaLabel(), disabled: this.disabled, onClicked: this._handleIconButtonClick })))), this.helperText && (index.h("div", { class: "inline-message-wrapper" }, index.h("wdpr-inline-message", { id: `${this._textFieldId}-helper-text`, variant: this.error ? 'error' : 'informational', size: "small", role: "status" }, this.helperText)))));
    }
    _getIconName() {
    ...
    _renderList() {
        ...
        return (index.h("div", { ref: el => (this._listEl = el), class: "listbox", role: "listbox", id: this._listId, tabindex: isScrollable ? 0 : undefined, "aria-label": this.a11yLabel, "aria-multiselectable": this.isMultiSelect ? 'true' : undefined }, index.h("slot", null)));
    }
    _renderListWrapper() {
        const listContainerClasses = this._isListVisible ? 'combobox-list-container-visible' : 'combobox-list-container-invisible';
        return (index.h("div", { class: `combobox-list-container ${listContainerClasses}`, style: { height: this._measuredMaxHeight } }, this._renderList()));
    }
...
exports.wdpr_combobox = WdprCombobox;
//# sourceMappingURL=wdpr-combobox.entry.cjs.js.map

//# sourceMappingURL=wdpr-combobox.cjs.entry.js.map