...
const WdprDatepicker = class {
     ...
     * @type {string}
     */
    min = date_utils.formatDateToString(new Date(1, 1, 1), this.format);
    /**
    ...
    dateInvalid;
    componentWillLoad() {
        this._internalDialogId = this.dialogId || `wdpr-datepicker-calendar-${utils.generateRandId()}`;
    ...
    _getValidationResult(date) {
        if (!date)
            return null;
        if (!date_utils.isValidDateString(date, this.format)) {
            return { type: 'invalid-format', message: 'Invalid date format' };
        }
        const formattedDate = date_utils.parseDateFromString(date, this.format);
        const minDate = this.min && date_utils.parseDateFromString(this.min, this.format);
        const maxDate = this.max && date_utils.parseDateFromString(this.max, this.format);
        if ((minDate && date_utils.isBeforeDay(formattedDate, minDate)) || (maxDate && date_utils.isAfterDay(formattedDate, maxDate))) {
    ...
    get _calendarWrapperClasses() {
        ...
        return bundleCjs.bundleCjsExports.twMerge(baseClasses, shadowClass, expandedClasses);
    }
...
exports.wdpr_datepicker = WdprDatepicker;
//# sourceMappingURL=wdpr-datepicker.entry.cjs.js.map

//# sourceMappingURL=wdpr-datepicker.cjs.entry.js.map