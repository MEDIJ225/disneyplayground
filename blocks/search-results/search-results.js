...
export default async function decorate(block) {
  ...
  const tabs = createElement('div', { class: 'search-results-tabs', role: 'tablist' }, [
    createElement('button', {
      ...
      type: 'button',
    }, placeholders.siteSearchResults || 'Site Search Results'),
    createElement('button', {
      role: 'tab',
      id: 'tab-blog',
      ...
      type: 'button',
    }, placeholders.blogSearchResults || 'Blog Search Results'),
  ]);
  ...
  container.append(tabs, resultsInfo, sitePanel, blogPanel);
  block.textContent = '';
  ...
  function renderList(list, parent, qTokens, isBlogResults = false) {
    parent.textContent = '';
    if (!list.length) {
      parent.append(createElement('p', { class: 'search-results-empty' }, placeholders.noResults || 'No results.'));
      return;
    }
    ...
    list.forEach((item) => {
      ...
      if (item.author) metaPieces.push(`${byText} ${item.author}`);
      ...
      const catsLine = Array.isArray(item.tags) && item.tags.length
        ? createElement('div', { class: 'search-results-cats' }, [
          createElement('span', { class: 'label' }, placeholders.categoriesLabel || 'Categories: '),
          ...item.tags.map((t, idx) => {
            ...
            const link = createElement('a', { href }, formattedCategory);
  ...
  function updateTabsCounts(siteCount, blogCount) {
    ...
    const siteLabel = placeholders.siteSearchResults || 'Site Search Results';
    const blogLabel = placeholders.blogSearchResults || 'Blog Search Results';
    siteTab.textContent = `${siteLabel} (${siteCount})`;
    blogTab.textContent = `${blogLabel} (${blogCount})`;
  }
  ...
  async function runSearch(query) {
    ...
    const resultsFoundLabel = placeholders.resultsFoundFor || 'Results Found for';
    resultsInfo.textContent = query ? `${site.length + blog.length} ${resultsFoundLabel} "${query}"` : '';
