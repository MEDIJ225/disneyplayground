...
function createCustomDropdown(id, title, placeholder, options) {
  ...
  function openDropdown() {
    ...
    if (menuOptions.length > 0) {
      ...
      const selectEvent = placeholder.includes('Event') ? placeholder : 'Select Event';
      const selectYear = placeholder.includes('Year') ? placeholder : 'Select Year';
      const isPlaceholder = currentSummary === selectEvent || currentSummary === selectYear;
  ...
  function setSelectedValue(value) {
    ...
    for (let i = 0; i < menuOptions.length; i += 1) {
      if (menuOptions[i].dataset.value === value) {
        ...
        // Focus management is handled by updateSelectedOption when dropdown is open
        break;
      }
    }
  }
...
function getEventSlugFromURL() {
  ...
  if (resultsIndex !== -1 && pathParts.length > resultsIndex + 1) {
    return pathParts[resultsIndex + 1]; // event slug
  }
 ...
 */
async function createGetResultsButton() {
  const placeholders = await fetchPlaceholders();
  return createElement('button', {
    type: 'button',
    class: ['results-filter-button', 'button'],
    disabled: true,
  }, placeholders.getResults || 'Get Results');
}
...
export default async function decorate(block) {
  try {
    ...
    if (!resultsIndex || resultsIndex.length === 0) {
      block.innerHTML = `<p>${placeholders.noResultsDataAvailable || 'No results data available.'}</p>`;
      return;
    }
    ...
    if (events.length === 0) {
      block.innerHTML = `<p>${placeholders.noEventsFound || 'No events found in results data.'}</p>`;
      return;
    }
    ...
    const eventDropdown = createCustomDropdown('event-select', placeholders.event || 'Event', placeholders.selectEvent || 'Select Event', eventOptions);

    const yearDropdown = createCustomDropdown('year-select', placeholders.year || 'Year', placeholders.selectYear || 'Select Year', []);

    const getResultsButton = await createGetResultsButton();
    ...
    if (eventSlug) {
      const preselected = eventDropdown.container.querySelector(
        `.custom-select-option[data-value="${eventSlug}"]`,
      );
      if (preselected) {
        preselected.setAttribute('aria-selected', 'true');
        eventDropdown.container.querySelector('.custom-select-summary').textContent = preselected.textContent;
      }
    }
    ...
    getResultsButton.addEventListener('click', () => {
      if (selectedEventSlug && selectedYear) {
        window.s_wdpro.trackClick(this, 'RD_PS_Results_GetResults');
    ...
    const placeholders = await fetchPlaceholders();
    block.innerHTML = `<p>${placeholders.errorLoadingResultsFilter || 'Error loading results filter. Please try again later.'}</p>`;
  }
}
